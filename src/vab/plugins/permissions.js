...
router.beforeEach(async (to, from, next) => {
  ...
  // 是否为纯净模式
  if (to.query.pure && to.query.pure == '1') {
    store.getters['settings/theme'].layout = 'pure'
    store.dispatch('settings/updateTheme')
  ...
  if (hasToken) {
    ...
    } else if (to.path === '/sso/oauth2') {
      location.href = oauth2.getUserAuthorize()
