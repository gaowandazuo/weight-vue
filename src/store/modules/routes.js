...
import { parseJsonToTree } from '@/utils/bspToWuu'
import { authentication, rolesControl, menuTypeId } from '@/config'
...
const actions = {
   ...
   * @returns
   */
  async setRoutes({ commit, rootGetters }, mode = 'none') {
    // 默认前端路由
    ...
    if (authentication === 'all') {
      ...
      routes = convertRouter(parseJsonToTree(list))
    }
